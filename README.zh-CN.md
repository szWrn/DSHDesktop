...
> 请补充一张运行截图或 GIF（建议放到 `docs/screenshot.png`），展示「启动 → 服务在线 → 嵌入 DSH 界面」的完整流程。
...
- **Windows**（当前版本仅支持 Windows）
- **Node.js**（需要 `npx` 在 PATH 中；首次启动会通过 `npx` 拉取 `@deepseek-ai/dsh`，需要联网）
- 确保 `127.0.0.1:3080` 端口空闲

> **平台说明**：目前后端进程管理依赖 `taskkill` / `netstat`，因此只支持 Windows。macOS / Linux 支持已列入计划，欢迎贡献。
...
- **停止**：通过 `netstat -ano` 找到占用 3080 端口的进程，用 `taskkill` 结束。
...
**Q：支持 macOS / Linux 吗？**
暂不支持。当前版本仅支持 Windows，跨平台支持是后续计划。
...
| 进程管理       | Rust `std::process` + `netstat` / `taskkill`                |
...
Good first issues：macOS / Linux 进程管理移植、安装器图标与元数据完善、自动发布 CI。
